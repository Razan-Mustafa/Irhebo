    ...
    <div class="content">
        <div class="main-content">
            <div class="block justify-between page-header md:flex">
                <div>
                    <h3 class="text-[1.125rem] font-semibold">{{ __('privacy_policy') }}</h3>
                </div>
                <ol class="flex items-center whitespace-nowrap">
                    ...
                    <li class="text-[0.813rem] font-semibold text-primary">{{ __('privacy_policy') }}</li>
                </ol>
            </div>
        </div>
                ...
                <div class="col-span-12">
                    <div class="bg-white shadow-md rounded-lg p-6">
                        <div class="border-b pb-4 mb-4">
                            <h5 class="text-lg font-semibold">{{ __('privacy_policy') }}</h5>
                        </div>
                        <form action="{{ route('general.updatePrivacyPolicy') }}" method="POST"
                                ...
                                @foreach ($privacy as $key => $value)
                                    <div class="col-span-12 md:col-span-12">
                                        <label class="block text-sm font-medium text-gray-700 mb-3">
                                            {{ $key === 'privacy_en' ? __('privacy_en') : __('privacy_ar') }}
                                        </label>
    ...
    <script>
        $(document).ready(function() {
            ...
            $.validator.addMethod("summernoteRequired", function(value, element) {
                ...
                return $(element).attr('id') === 'privacy_en' ?
                    "{{ __('validation.privacy_en.required') }}" :
                    "{{ __('validation.privacy_ar.required') }}";
            });
            ...
            $('form').on('submit', function() {
                $('#privacy_en').val($('#privacy_en').summernote('code'));
                $('#privacy_ar').val($('#privacy_ar').summernote('code'));
                return true;
            });
        });
    </script>
